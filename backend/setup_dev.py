...
async def main():
    ...
    async with async_session_factory() as session:
        ...
        # ── Objects ──
        objects = [
            Object(name="Intermarche Szczytno", description="Supermarket - regaly chlodnicze, mroznie", is_active=True),
            Object(name="Biedronka Olsztyn", description="Dyskont spozywczy - lady chlodnicze", is_active=True),
            Object(name="Magazyn Centralny", description="Magazyn chlodniczy - duze komory mroznicze", is_active=True),
        ]
        ...
        # ── Sensors ──
        sdata = [
            (objects[0].id, "Komora chlodnicza A1", "frigo/intermarche/komora-a1", 3.8, 60),
            (objects[0].id, "Mroznia B1", "frigo/intermarche/mroznia-b1", -18.2, 60),
            (objects[1].id, "Lada miesna", "frigo/biedronka/lada-miesna", 2.1, 90),
            (objects[1].id, "Lada nabialowa", "frigo/biedronka/lada-nabialowa", 4.5, 90),
            (objects[2].id, "Komora mroznicza G1", "frigo/magazyn/komora-g1", -22.0, 120),
            (objects[2].id, "Komora chlodnicza G2", "frigo/magazyn/komora-g2", 2.0, 120),
        ]
        sensors = []
        for oid, nm, topic, temp, timeout in sdata:
            s = Sensor(name=nm, mqtt_topic=topic, current_temperature=temp, last_message_at=now,
